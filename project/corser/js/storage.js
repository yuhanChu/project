//Object.prototype.unset = function (key) {
...
(function(window){
	...
	window.LS = !LS ? notSupport : {
		set : function(){
			...
			var lifeTime = arguments[2];
			var lifesTime = LS.getItem('lifesTime');

			var	lifesTimeObj = lifesTime != null ? JSON.parse(lifesTime) : {};

			if(typeof(lifesTimeObj) != 'object') {
				lifesTimeObj = {};
			}
			var now = Date.parse(new Date());
			lifesTimeObj[key] = lifeTime ? now + lifeTime*1000*60*60 : now + 60*60*24*1000;

			if( this.get(key) !== undefined )
				this.remove(key);
			LS.setItem(key, value);
			LS.setItem('lifesTime', JSON.stringify(lifesTimeObj));
		...
		get : function(key) {
			var lifesTime = LS.getItem('lifesTime');
			if(lifesTime !== null) {
				lifesTimeObj = JSON.parse(lifesTime);
				if(typeof(lifesTimeObj) == 'object' && lifesTimeObj[key]) {
					var valueLifesTime = lifesTimeObj[key];
					var timestamp = Date.parse(new Date());
					if(timestamp > valueLifesTime) {
						LS.removeItem(key);
						this.length = LS.length;
						//lifesTimeObj = lifesTimeObj.unset(key);
						//LS.setItem('lifesTime', JSON.stringify(lifesTimeObj));
					}
				}
			}
